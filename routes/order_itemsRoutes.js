...
router.post('/order_items', (req, res) => {
    const { order_item_id, order_id, ticket_id, quantity, subtotal } = req.body;
    const sql = `INSERT INTO order_items (order_item_id, order_id, ticket_id, quantity, subtotal) VALUES (?, ?, ?,
    ?, ?)`;
    db.query(sql, [order_item_id, order_id, ticket_id, quantity, subtotal], (err, result) => {
        if (err) {
            return res.status(500).send(err);
        }
        res.status(201).send({ order_item_id: result.insertId, order_item_id, order_id, ticket_id, quantity, subtotal });
    });
});
...
router.put('/order_items/:order_item_id', (req, res) => {
    const { order_item_id, order_id, ticket_id, quantity, subtotal } = req.body;
    const sql = `UPDATE order_items SET order_item_id = ?, order_id = ?, ticket_id = ?, 
    quantity = ?, subtotal = ? WHERE order_item_id = ?`;
    db.query(sql, [order_item_id, order_id, ticket_id, quantity, subtotal], (err, result) => {
        if (err) {
            return res.status(500).send(err);
        }
        ...
        res.status(200).send({ order_item_id: req.params.order_item_id, order_id, 
            ticket_id, quantity, subtotal });
    });
});
